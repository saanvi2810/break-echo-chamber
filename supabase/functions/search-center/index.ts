...
async function searchVertexAI(topic: string): Promise<Article[]> {
  ...
  console.log(`[CENTER] Searching Vertex AI searchLite: ${fullQuery.slice(0, 100)}...`);
  ...
  if (!response.ok) {
    ...
    console.error(`[CENTER] Vertex AI searchLite error: ${response.status} - ${errorText.slice(0, 300)}`);
    throw new Error(`Vertex AI ${response.status}`);
  }

  const data = await response.json();
  const results = data.results || [];
  console.log(`[CENTER] Vertex AI searchLite returned ${results.length} results`);
