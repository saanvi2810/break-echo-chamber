...
async function searchVertexAI(topic: string): Promise<Article[]> {
  ...
  console.log(`[RIGHT] Searching Vertex AI searchLite: ${fullQuery.slice(0, 100)}...`);
  ...
  if (!response.ok) {
    ...
    console.error(`[RIGHT] Vertex AI searchLite error: ${response.status} - ${errorText.slice(0, 300)}`);
    throw new Error(`Vertex AI ${response.status}`);
  }

  const data = await response.json();
  const results = data.results || [];
  console.log(`[RIGHT] Vertex AI searchLite returned ${results.length} results`);
